...
namespace moisesToolkit.MercadoPago.NetCore.JsonMaps
{
    internal class RefundMap : IJsonTypeConfiguration<Refund>
    {
        public void Configure(JsonTypeBuilder<Refund> jsonTypeBuilder)
        {
            ...
            jsonTypeBuilder.Property(x => x.Amount).HasFieldName("amount");
            jsonTypeBuilder.Property(x => x.PaymentId).HasFieldName("payment_id");
            jsonTypeBuilder.Property(x => x.DateCreated).HasFieldName("date_created");
            jsonTypeBuilder.Property(x => x.Errors).HasFieldName("errors");
        }
    }
}
